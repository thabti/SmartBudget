...
module.exports = function ({ Plugin, types: t}) {
  ...
  function importResolver(path, state) {
    ...
    const extensions = (path.opts.extensions || ['css', 'scss']).map(function(ext) {
        ...
        exit(path, state) {
          if (css != null) {
            ...
            if (classes.length === 0) {
              ...
              // classes
              console.log('APPLYING AN ARRAY:', classes);
              const arrayExpression = t.ArrayExpression(classes);
              // const arrayIdentifierExpression = t.identifier(arrayExpression);
